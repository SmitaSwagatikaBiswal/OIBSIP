...
class WeatherApp:
    def __init__(self, root):
        ...
        self.unit = "metric"
        title = tk.Label(root,text="Advanced Weather App",font=("Arial",20,"bold"))
        title.pack(pady=10)
        top = tk.Frame(root)
        top.pack()
        self.city_entry = tk.Entry(top,width=30,font=("Arial",12))
        self.city_entry.pack(side=tk.LEFT,padx=5)
        tk.Button(top,text="Get Weather",command=self.get_weather).pack(side=tk.LEFT)
        tk.Button(top,text="Use My Location",command=self.detect_location).pack(side=tk.LEFT,padx=5)
        self.toggle_btn = tk.Button(top,text="Switch to °F",command=self.toggle_units)
        self.toggle_btn.pack(side=tk.LEFT)
        self.error_label = tk.Label(root,fg="red",font=("Arial",11))
        self.error_label.pack()
        self.result = tk.Label(root,font=("Arial",12),justify=tk.LEFT)
        self.result.pack()
        self.icon_label = tk.Label(root)
        self.icon_label.pack()
        tk.Label(root,text="Next 6 Hours",font=("Arial",14,"bold")).pack()
        self.hour_frame = tk.Frame(root)
        self.hour_frame.pack()
        tk.Label(root,text="Next 5 Days",font=("Arial",14,"bold")).pack()
    ...
    def get_weather(self):
        ...
        try:
            weather=requests.get(CURRENT_URL,params=params,timeout=10)
            ...
            if self.unit=="metric":
                ...
                f=temp
                c=(temp-32)*5/9
            text=f"""City : {city}Temperature :{c:.1f} °C{f:.1f} °FHumidity : {humidity} %Wind Speed : {wind}Condition : {desc}"""
