...
class PasswordGenerator:
    def __init__(self, root):
        ...
        self.build_gui()
    def build_gui(self):
        title = tk.Label(self.root,text="Advanced Password Generator",font=("Arial", 18, "bold"))
        title.pack(pady=10)
        frame1 = tk.Frame(self.root)
        frame1.pack(pady=5)
        tk.Label(frame1, text="Password Length:", font=("Arial", 11)).pack(side=tk.LEFT)
        self.length_var = tk.IntVar(value=12)
        self.length_spin = tk.Spinbox(frame1,from_=8,to=64,width=5,textvariable=self.length_var)
        ...
        tk.Checkbutton(options,text="Uppercase (A-Z)",variable=self.upper).pack(anchor="w")
        tk.Checkbutton(options,text="Lowercase (a-z)",variable=self.lower).pack(anchor="w")
        tk.Checkbutton(options,text="Numbers (0-9)",variable=self.number).pack(anchor="w")
        tk.Checkbutton(options,text="Symbols (!@#$...)",variable=self.symbol).pack(anchor="w")
        tk.Checkbutton(options,text="Exclude Ambiguous Characters (0 O l 1 I)",variable=self.exclude).pack(anchor="w")
        button_frame = tk.Frame(self.root)
        button_frame.pack(pady=10)
        tk.Button(button_frame,text="Generate Password",command=self.generate_password,width=20,bg="green",fg="white").pack(side=tk.LEFT, padx=5)
        tk.Button(button_frame,text="Copy Password",command=self.copy_password,width=15).pack(side=tk.LEFT)
        self.password_var = tk.StringVar()
        entry = tk.Entry(self.root,textvariable=self.password_var,font=("Consolas", 14),width=40,justify="center")
        entry.pack(pady=10)
        tk.Label(self.root,text="Password Strength",font=("Arial", 11)).pack()
        self.progress = ttk.Progressbar(self.root,length=300,maximum=100 )
        self.progress.pack(pady=5)
        self.strength_label = tk.Label(self.root,text="",font=("Arial", 12, "bold"))
        self.strength_label.pack()
        history_frame = tk.LabelFrame(self.root,text="Last 5 Passwords")
        history_frame.pack(fill="both", padx=20, pady=20)
        self.history_list = tk.Listbox(history_frame,height=5,font=("Consolas", 11))
